...
def main(args=None):
    ...
    if args.edit_mode == 'trajectory':
        print("[#] Sampling with trajectory...")
        motion_template = torch.zeros((args.batch_size, model.njoints, model.nfeats, n_frames), device=dist_util.dev())
        model_kwargs['y']['inpainting_mask'] = torch.tensor(humanml_utils.HML_TRAJ_MASK, dtype=torch.bool,
                                                            device=motion_template.device)  # True is lower body data
        model_kwargs['y']['inpainting_mask'] = model_kwargs['y']['inpainting_mask'].unsqueeze(0).unsqueeze(
            -1).unsqueeze(-1).repeat(motion_template.shape[0], 1, motion_template.shape[2], motion_template.shape[3])
        ...
        model_kwargs['y']['lengths'] = (model_kwargs['y']['lengths'] * 0) + n_frames # convert to numpy array for consistency
    else:
        pass
