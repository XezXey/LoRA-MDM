...
class StyleMotionDataset(Dataset):
    ...
    def __init__(self, styles, mode, motion_type_to_exclude=[], split_file=None):
        ...
        path = "/home/mint/Dev/CR7/human_motions/LoRA-MDM/my_scripts/prepare_training_data/data_for_lora/"
        if (split_file is None) or (not os.path.exists(split_file)):
            split_file = path + "/train/" +  "train_demo_cr7_1.txt" 
            print("[#] Using default split file: ", split_file)
        else:
            print("[#] Using custom split file: ", split_file)
        ...
        dict_path = path + "MintStyle_name_dict.txt"
        ...
        motion_dir = path + "/new_joint_vecs"
        ...
        enumerator = enumerate(
            track(
                id_list,
                f"Loading MINTSTYLE {len(styles)} {split_file.split('/')[-1].split('.')[0]}"
            ))
        maxdata = 1e10 
