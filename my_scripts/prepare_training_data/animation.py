...
parser.add_argument('--tgt_ani_dir', type=str, required=True)
...
if __name__ == '__main__':
    ...
    for npy_file in tqdm(npy_files):
        ...
        # Save to json file with {'motions': [data.tolist()]} to compatible with the HMVisualizer
        data = data.astype(np.float32)
        data = data.transpose(1, 2, 0)   # [T, 22, 3] -> [22, 3, T]
        out = {
            'motions': data[None, ...].tolist(),
            'prompts': ["-"],
        }
        with open(save_path[:-4] + '.json', 'w') as f:
            json.dump(out, f)