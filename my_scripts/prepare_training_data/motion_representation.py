...
save_dir1 = f"./{args.save_dir}/new_joints/"
save_dir2 = f"./{args.save_dir}/new_joint_vecs/"
...
example_data = np.load(
    os.path.join("/data/mint/Motion_Dataset/HumanML3D/new_joints/", example_id + ".npy")
)
# example_data = np.load(os.path.join('/data/mint/Motion_Dataset/HumanML3D/new_joint_vecs/', example_id + '.npy'))
...
source_list = [os.path.basename(i) for i in glob.glob(f"{data_dir}/*.npy")]
frame_num = 0
for source_file in tqdm(source_list):
    print("[#] Processing: ", source_file)
    print("[#] Pose shape: ", np.load(os.path.join(data_dir, source_file)).shape)
    x = np.load(os.path.join(data_dir, source_file))  # T x J x 3; J = 45
    # Plot joint in 3D using plotly and save into html
